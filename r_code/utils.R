...
gtheme = theme_minimal() + 
  theme(text = element_text(size=font.size, family = 'sans', color=grey.color), 
        panel.background = element_rect(fill = 'white'),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        line = element_line(linewidth = px2pt),
        axis.line.x = element_line(linewidth = 0.8 * px2pt, linetype = "solid", colour = grey.color),
        axis.line.y = element_line(linewidth = 0.8 * px2pt, linetype = "solid", colour = grey.color),
        axis.ticks.x = element_line(linewidth = 0.8 * px2pt, linetype = "solid", colour = grey.color),
        axis.ticks.y = element_line(linewidth = 0.8 * px2pt, linetype = "solid", colour = grey.color),
        axis.text.x=element_text(size = font.size, colour = grey.color),
        axis.text.y=element_text(size = font.size, colour = grey.color),
        axis.title.x = element_text(size = font.size),
        axis.title.y = element_text(size = font.size),
        strip.text.x = element_text(size = font.size),
        strip.text.y = element_text(size = font.size, color=grey.color),
...
find.fst.idx = function(vals, ge.val) {
  x = which(vals >= ge.val)
  if (length(x) > 0) {
    return(x[1])
  } else {
